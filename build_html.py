...
ON_DEMAND_RE = re.compile(
    r"^On demand\s*\|\s*Present(?:ing|ed) live\s+(?P<date>\d{1,2}\s+\w+\s+\d{4})$"
)
...
#asteroids-btn svg { width: 20px; height: 20px; }
#qr-modal-img {
  display: block;
...
.agenda-row.agenda-now { background: #fff4f4; }
.agenda-row.agenda-now .agenda-time-start { color: #e63946; }
.agenda-row.agenda-now .agenda-time-col::after {
  content: 'now';
  display: block;
  width: fit-content;
  margin: 3px 0 0 auto;
...
function _renderAgendaDay(day) {
  ...
  for (const t of talks) {
    ...
    rows += `<div class="agenda-row talk${plenary?' agenda-row--plenary':''}" data-search="${search}" data-id="${he(id)}" data-start="${t.start_min}" data-end="${_timeBounds(t)[1]}" ${_modalAttrs(t, day.label)} style="border-left-color:${color}">` +
  ...
  return `<div class="cal-agenda" data-day="${day.date_iso}">${rows}</div>`;
}
...
function updateNowLine() {
  ...
  // Agenda (mobile) view: highlight the rows of talks happening right now.
  document.querySelectorAll('.cal-agenda').forEach(agenda => {
    const live = agenda.dataset.day === dateIso;
    agenda.querySelectorAll('.agenda-row').forEach(row => {
...
<div class="topbar">
  ...
  <div class="topbar-right">
    ...
    <button id="asteroids-btn" onclick="window.location.href='asteroids/'" title="Asteroids">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="2 5.5 20 13" stroke-width="0.94" stroke="currentColor" aria-hidden="true">
        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 9v3M6.75 10.5h3M16.875 9.75h.008v.008h-.008V9.75ZM15 11.25h.008v.008H15v-.008ZM8.5 7.5h7c1.5 0 2.5.5 3 1.7c.6 1.4 2.2 3.9 2.2 5.4c0 1.3-.9 2.3-2.1 2.3c-1.1 0-2.2-1.3-3.1-2.5c-.9-1.1-2.1-1.3-3.5-1.3c-1.4 0-2.6.2-3.5 1.3c-.9 1.2-2 2.5-3.1 2.5c-1.2 0-2.1-1-2.1-2.3c0-1.5 1.6-4 2.2-5.4c.5-1.2 1.5-1.7 3-1.7Z" />
      </svg>
    </button>
